var ratingsAverage = document.getElementById("rating");
var ratingsQuantity = document.getElementById("reviews");
var difficulty = document.getElementById("difficulty");
var category = document.getElementById("region");
...
var ingredients = document.getElementById("ingredients"); // Generative
var instructions = document.getElementById("instructions"); // Generative
...
var tips = document.getElementById("tips"); // Generative
...
var lastRecipieIndex = -1;
var currentRecipieIndex = Math.floor(Math.random() * recipies.length);

var ingredientsHTML = "";
var instructionsHTML = "";
var tipsHTML = "";

function addIngredient(number, ingredient) {
  ingredientsHTML += `
  <li class="d-flex align-items-start mb-3">
    <div class="w-1.5rem h-1.5rem me-2 bg-orange-500 rounded-circle d-flex align-items-center justify-content-center text-white fw-bold flex-shrink-0">${number}</div>
    <span>${ingredient}</span>
  </li>
  `;
}
function addInstruction(number, instruction) {
  instructionsHTML += `
  <!-- instruction -->
  <div class="d-flex align-items-start">
    <div class="w-3rem h-3rem bg-orange-500 rounded-4 d-flex align-items-center justify-content-center text-white fs-4 fw-bold me-3 flex-shrink-0">${number}</div>
...
function addTip(tip) {
  tipsHTML += `
  <div class="d-flex align-items-start p-3 bg-amber-50 rounded-4 border-start border-4 border-amber-400 mb-3">
    <i class="fa-solid fa-circle-check text-amber-600 fs-4 me-3 mt-1"></i>
    <p class="m-0">${tip}</p>
  </div>
  `;
}

function displayRecipie(index) {
  var recipie = recipies[index];
  ingredientsHTML = "";
  instructionsHTML = "";
  tipsHTML = "";

  // simple
  ratingsAverage.textContent = recipie.ratingsAverage;
  ratingsQuantity.textContent = `(${recipie.ratingsQuantity} reviews)`;
  difficulty.textContent = recipie.difficulty;
  category.textContent = recipie.category;
  recipeName.textContent = recipie.name;
  description.textContent = recipie.description;
  timeWarning.classList.toggle("d-none", recipie.totalTime <= 45);
  image.src = recipie.imgCover;
  image.alt = recipie.image;
  prepTime.textContent = recipie.prepTime;
  cookTime.textContent = recipie.cookTime;
  servings.textContent = recipie.servings;
  calories.textContent = recipie.nutrition.calories;
  protein.textContent = recipie.nutrition.protein;
  carbs.textContent = recipie.nutrition.carbs;
  fat.textContent = recipie.nutrition.fat;
  fiber.textContent = recipie.nutrition.fiber;
  sodium.textContent = recipie.nutrition.sodium;

  // generative
  for (var i = 0; i < recipie.ingredients.length; i++) {
    addIngredient(i + 1, recipie.ingredients[i]);
  }

  ingredients.innerHTML = ingredientsHTML;

  for (var i = 0; i < recipie.instructions.length; i++) {
    addInstruction(i + 1, recipie.instructions[i]);
  }

  instructions.innerHTML = instructionsHTML;

  for (var i = 0; i < recipie.tips.length; i++) {
    addTip(recipie.tips[i]);
  }

  tips.innerHTML = tipsHTML;
}

function showAnotherRecipie() {
  do {
    lastRecipieIndex = currentRecipieIndex;
    currentRecipieIndex = Math.floor(Math.random() * recipies.length);
    displayRecipie(currentRecipieIndex);
  } while (currentRecipieIndex === lastRecipieIndex);
}
